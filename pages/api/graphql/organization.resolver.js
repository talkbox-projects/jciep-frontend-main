...
export default {
  ...
  Mutation: {
    OrganizationSubmissionCreate: async (_parent, params, context) => {
      ...
      if (organization) {
        let organizationSubmission = await new OrganizationSubmission({
          ...
          missionNVision: params?.input?.missionNVision,
          targetGroup: params?.input?.targetGroup,
          targetGroupDisabilities: params?.input?.targetGroupDisabilities,
          targetGroupDisabilitiesOther: params?.input?.targetGroupDisabilitiesOther,
          website: params?.input.website,
          businessRegistration: params?.input.businessRegistration,
          industry: params?.input.industry,
          industryOther: params?.input.industryOther,
    ...
    OrganizationMemberJoin: async (_parent, { identityId, invitationCode }, context) => {
      ...
      const currIdentity = context?.auth?.identity;
      if (!checkIfAdmin(currIdentity) && !isIdentityUnderUser(identityId, context?.auth?.user)) {
        throw new Error("Permission Denied");
      }
